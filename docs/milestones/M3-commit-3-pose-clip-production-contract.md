# M3 Commit 3.0 — Multi-frame PoseClip Production Contract
...
因此修改任意帧的姿态、接触、Anchor 要求或引用资产，只会使该帧的生成缓存失效；其他帧仍可复用、独立重试和独立审核。
...
Golden/negative tests 固定四帧 rabbit run 请求的 FrameSpec Hash、Generation Input Hash 与 Production Request Hash，并覆盖：
