...
from run_asset_qa import inspect_image  # noqa: E402
...
class AssetQaTest(unittest.TestCase):
    def test_passes_valid_alpha_png_with_normalized_anchors(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            image.save(root / "processed" / "valid.png")
            (root / "anchors" / "valid.json").write_text(json.dumps({
                "anchors": {"foot": {"x": 0.5, "y": 0.9}, "center": {"x": 0.5, "y": 0.5}}
            }), encoding="utf-8")
            self.assertEqual(inspect_image(root, "valid", "processed/valid.png")["status"], "passed")
