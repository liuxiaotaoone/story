...
function poseClip(id: string, entityType = 'rabbit') {
  return {
    id, entityType, action: 'run', loop: true, direction: 'left' as const,
    frames: [{
...
function bindingCatalog(clipId = 'rabbit.run-left'): ResolvedAssetCatalog {
  const clip = poseClip(clipId);
  return {
    schemaVersion: '1.0.0', mode: 'experiment', productionReady: false, catalogHash: HASH,
    assets: {schemaVersion: '1.0.0', assets: [visualAsset(`${clipId}.frame`)]},
    poseClips: [clip], environments: [],
    entityDefinitions: [{
...
describe('Final Compiler input and asset binding hardening', () => {
  ...
  it('drops optional actions explicitly with stable diagnostics', () => {
    const required = {
      ...
      minDurationFrames: 10, requiredPoseClipIds: ['rabbit.run-left'],
    };
